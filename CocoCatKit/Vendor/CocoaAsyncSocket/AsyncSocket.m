...
- (void)close
{
	...
	// Do not access any instance variables after calling socketDidDisconnect.
...
- (void)completeCurrentRead
{
	...
	{
		[theDelegate socket:self didReadData:result withTag:theCurrentRead->tag];
	}
...
- (void)doSendBytes
{
	...
	else if (totalBytesWritten > 0)
	{
		...
		if ([theDelegate respondsToSelector:@selector(socket:didWritePartialDataOfLength:tag:)])
		{
			[theDelegate socket:self didWritePartialDataOfLength:totalBytesWritten tag:theCurrentWrite->tag];
		}
	}
}
...
- (void)onTLSHandshakeSuccessful
{
	if((theFlags & kStartingReadTLS) && (theFlags & kStartingWriteTLS))
	{
		...
		if([theDelegate respondsToSelector:@selector(socketDidSecure:)])
		{
